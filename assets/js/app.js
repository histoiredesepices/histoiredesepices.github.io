...
function openDishModal(dishId, triggerEl) {
    ...
    if (detailsEl) {
        ...
        // Allergens — always shown in modal regardless of settings flag
        const allergenLabel =
        ...
        // Calories (respects settings flag)
        if (C.settings.show_calories && dish.calories_kcal != null) {
            rows += `
                <div class="dish-modal__detail-row">
                    <div class="dish-modal__detail-icon">
    ...
    if (ctaEl) {
        ...
        if (waPhone) {
            ...
            const href = waUrl(waPhone, msg);
            const btnLabel =
                lang === 'fr' ? 'Commander ce plat sur WhatsApp' : 'Order this dish on WhatsApp';
            ctaEl.innerHTML = `<a href="${esc(href)}" target="_blank" rel="noopener">${waSvg('white', 20)} ${esc(btnLabel)}</a>`;
        } else {
            ctaEl.innerHTML = '';
        }
    }
...
function initDishModal() {
    ...
    // Delegated click — open modal on any [data-dish] card
    document.addEventListener('click', (e) => {
        const modal = $('dish-modal');
        if (modal && !modal.hidden) return; // modal already open, let its own handlers work
